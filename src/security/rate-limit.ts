 ...
 */
export const getRateLimitStatePath = (scope: 'user' | 'repo'): string => {
 ...
 */
export const loadRateLimitState = (scope: 'user' | 'repo'): RateLimitState => {
 ...
 */
export const saveRateLimitState = (scope: 'user' | 'repo', state: RateLimitState): void => {
  const statePath = getRateLimitStatePath(scope);
 ...
 * @param scope - Scope of rate limiting
 */
export const recordInjectionAttempt = (entityId: string, scope: 'user' | 'repo'): void => {
  const state = loadRateLimitState(scope);
  const now = Date.now();
 ...
 * ```
 */
export const resetRateLimit = (entityId: string, scope: 'user' | 'repo'): void => {
  const state = loadRateLimitState(scope);
 ...
 */
export const getTrackedEntities = (scope: 'user' | 'repo'): string[] => {
  const state = loadRateLimitState(scope);
 ...
 */
export const cleanupExpiredEntries = (scope: 'user' | 'repo'): number => {
  const securityConfig = loadSecurityConfig();
  const rateLimitConfig =
    scope === 'user' ? securityConfig.rateLimits.perUser : securityConfig.rateLimits.perRepo;
 ...
 */
export const isBlocked = (entityId: string, scope: 'user' | 'repo'): boolean => {
  const securityConfig = loadSecurityConfig();
  const rateLimitConfig =
    scope === 'user' ? securityConfig.rateLimits.perUser : securityConfig.rateLimits.perRepo;
