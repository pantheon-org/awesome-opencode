#!/usr/bin/env bun
/**
 * Security History Analysis Tool
 *
 * Analyzes historical security data to identify trends, patterns, and anomalies.
 * Provides insights for improving security posture and detection rules.
 *
 * Usage:
 *   bun run src/bin/analyze-security-history.ts [--days 90] [--format json|markdown]
 *
 * Options:
 ...
 */
function analyzeTrends(days: number): {
  trend: 'increasing' | 'decreasing' | 'stable';
  weeklyAverage: number;
  monthlyAverage: number;
  changePercent: number;
 ...
 */
function generateRecommendations(results: Omit<AnalysisResults, 'recommendations'>): string[] {
 ...
 */
function analyzeSecurityHistory(days: number): AnalysisResults {
 ...
 */
function formatAsMarkdown(results: AnalysisResults): string {
